...
def exportar_asistencia_excel(request):
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Asistencia Semanal"

    encabezados = [
        "Empleado", "Semana", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes",
        "Total Almuerzo", "Total Comisión", "Total Permiso", "Total Trabajadas"
    ]
    ws.append(encabezados)
    ...
    for empleado in empleados:
        ...
        for semana_inicio in semanas:
            ...
            semana_label = f"{semana_inicio.strftime('%d/%m/%Y')} - {semana_fin.strftime('%d/%m/%Y')}"
            fila = [empleado.nombre_completo, semana_label]

            total_almuerzo = timedelta()
            total_comision = timedelta()
            total_permiso = timedelta()
            total_trabajadas = timedelta()
            ...
            for dia_offset in range(5):
                ...
                fila.append(ReporteService.formatear_dia_asistencia(datos_dia))
                horas = ReporteService.calcular_horas_empleado(datos_dia)
                total_almuerzo += horas.get('_almuerzo', timedelta())
                total_comision += horas.get('_comision', timedelta())
                total_permiso += horas.get('_permiso', timedelta())
                total_trabajadas += horas.get('_trabajadas', timedelta())

            fila.extend([
                ReporteService.strfdelta(total_almuerzo),
                ReporteService.strfdelta(total_comision),
                ReporteService.strfdelta(total_permiso),
                ReporteService.strfdelta(total_trabajadas),
            ])
            ws.append(fila)
    ...
    if ws.max_row > 1:
        tabla = Table(
            displayName="RegistroAsistenciaSemanal",
    ...
    response['Content-Disposition'] = 'attachment; filename=asistencia_semanal.xlsx'
    wb.save(response)
    return response
