...
class ReporteService:
    ...
    def calcular_horas_empleado(data):
        ...
        return {
            ...
            '_comision': comision,
            '_permiso': permiso,
            '_trabajadas': trabajadas,
        }

    @staticmethod
    def _get_times(data, key):
        k = key.lower()
        for existente, tiempos in data.items():
            if str(existente).lower() == k:
                return tiempos
    ...
    def formatear_dia_asistencia(data):
        ...
        orden_tipos = [
            ...
            'Salida por comisión', 'Entrada por comisión',
            'Salida por otros', 'Entrada por otros'
        ]

        for tipo in orden_tipos:
            tiempos = ReporteService._get_times(data, tipo)
            if tiempos:
                partes.append(f"{tipo} {tiempos[0].strftime('%H:%M')}")

        return '; '.join(partes)

    @staticmethod
    def obtener_semanas_en_rango(inicio, fin):
        semanas = []
    ...
    @staticmethod
    def obtener_registros_semanales(inicio, fin):
        datos_diarios = ReporteService.obtener_datos_resumen()
        datos_semanales = defaultdict(lambda: defaultdict(dict))
        ...
        for (id_empleado, fecha), data in datos_diarios.items():
            ...
            datos_semanales[id_empleado]['empleado'] = data['empleado']
            datos_semanales[id_empleado][semana_inicio][fecha] = data
